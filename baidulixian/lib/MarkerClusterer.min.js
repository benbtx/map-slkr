...
(function() {
    ...
    var MarkerClusterer =
        ...
        (BMapLib.MarkerClusterer = function(map, options) {
            ...
            //提升至map 中
            this._map._markers = this._markers;
            this._map._clusters = this._clusters;

            var opts = options || {};
            this._mapPoint = opts["mapPoint"]; //自定义
            this._person = opts["person"]; //自定义
            this._icons = opts["icons"]; //自定义

            this._isdiffcluster = opts["isdiffcluster"]; //自定义 是否开启不同类聚合
    ...
    Cluster.prototype.updateClusterMarker = function() {
        ...
        //更新 个数和 图标颜色
        //图标规则
        this._markerClusterer._person; //自定义
        this._markerClusterer._icons; //自定义
        if (this._markerClusterer._isdiffcluster) {
            // this._clusterMarker.setIcon(new BMap.Icon("/jsdemo/img/fox.gif"));
            // this._clusterMarker texticonoverlay
            this._markers; //当前cluster包含的maker  每个又包括person color eventype
            this._markers.forEach(element => {
                //只有一个对象，且是主对象 则使用它作为图标
                //有多个对象，且有主对象 则使用一个出现的主对象为图标
                //有多个对象，无主对象，则使用第一个为图标
                element.person;
                element.color;
                element.eventype;
            });
            // this._clusterMarker._styles[0].url = this._markerClusterer._icons[0].url;
        }
        ...
        //     // _belongText = this._markers[0].shequ ;//+"社区";
        //     _belongText = "";
        // } else if (zoom > 16) {
        ...
        this._clusterMarker.addEventListener("click", function(event) {
            ...
            // console.log(_this);
            console.log(event);
            console.log(_this._markerClusterer._clusters.length);
            console.log("单击聚合点");
            if (
                thatBounds.Ie == thatBounds.De &&
                thatBounds.Je == thatBounds.Ee
            ) {
                //落脚点分析等功能数据处理
                // window.showSingalTips({ a: 1 });
                console.log("弹出");
                var allpoint = _this._markerClusterer._mapPoint;
                // var curpoint = {};
                // window.ljd = {};
                for (var i in allpoint) {
                    if (
                        allpoint[i].longitude == thatBounds.Je &&
                        allpoint[i].latitude == thatBounds.Ie
                    ) {
                        // curpoint = allpoint[i];
                        // window.ljd = allpoint[i];
                        window.showSingalTips(allpoint[i]);
                        break;
                    }
                }
                // //打开对话框
                // const pt = new window.BMap.Point(thatBounds.Je, thatBounds.Ie);
                // this._map.centerAndZoom(pt, this._map.getZoom());
                // // var newstr = "23";

                // const infoWindow = new window.BMap.InfoWindow(
                //     "事件频次:" +
                //         curpoint.item.length +
                //         "&nbsp&nbsp<input type='button' style='border:1px solid #1490FF;color:#1490FF' value='查看详情' data-item='' onclick=' window.showSingalTips()'>",
                ...
                // this._map.openInfoWindow(infoWindow, pt);

                // _this._markerClusterer._mapPoint.forEach(element => {
                //     if (
                //         element.longitude == thatBounds.Je &&
                //         element.latitude == thatBounds.Ie
                //     ) {
                //         //打开对话框
                //         const pt = new window.BMap.Point(
                //             thatBounds.Je,
                //             thatBounds.Ie
                //         );
                //         this._map.centerAndZoom(pt, this._map.getZoom());
                ...
                //             // element.item[i].content;
                //             var reg = new RegExp(" {4}", "g");
                //             newstr +=
                //                 element.item[i].content.replace(reg, "<br>") +
                //                 "<br>";
                //         }

                //         const infoWindow = new window.BMap.InfoWindow(
                //             // this.myReplace(element.content, "    ", "<br>"),
                //             newstr || element.content,
                //             {
                //                 enableMessage: false
                //             }
                //         );
                ...
                thatMap.setViewport(thatBounds); //如果是坐标点相同的点聚合就不用再下钻
            }
        });
    };
